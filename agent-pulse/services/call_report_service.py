...
class CallReportService:
    ...
    async def _extract_call_id_from_report(self, report_data: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            # logger.error(f"report_data keys: {list(report_data.keys())}")
            # if "call" in report_data:
            #     logger.error(f"call keys: {list(report_data['call'].keys())}")
            #     if "assistantOverrides" in report_data["call"]:
            #         logger.error(f"assistantOverrides keys: {list(report_data['call']['assistantOverrides'].keys())}")
            #         if "variableValues" in report_data["call"]["assistantOverrides"]:
            #             logger.error(f"variableValues: {report_data['call']['assistantOverrides']['variableValues']}")
...
async def process_call_report(report_data: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convenience function to process call report data.
    ...
    async for db_session in get_db():
        service = CallReportService(db_session)
        return await service.process_end_call_report(report_data) 