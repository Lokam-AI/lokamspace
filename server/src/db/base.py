...
class Question(Base):
    ...
    id = Column(Integer, primary_key=True)
    subject = Column(String(64), nullable=False)  # category of the question
...
class Feedback(Base):
    ...
    score = Column(Integer)   # 1-10 score for the question
    sentiment = Column(String(16))  # Positive, Negative, Neutral
    key_points = Column(Text)       # Text
    tone = Column(String(16))       # Satisfied, Unsatisfied
    comments = Column(Text)         # Text
